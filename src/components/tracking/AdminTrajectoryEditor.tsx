...
function makeId() {
  _idCounter++;
  return `step_${_idCounter}_${Math.random().toString(36).slice(2, 7)}`;
}
...
export function AdminTrajectoryEditor({ requestId, initialSteps, mapboxToken }: Props) {
  ...
  const saveMutation = useAuraMutation<{ requestId: string; steps: any[] }, { success: true }>("admin.saveTrajectory", {
    onSuccess: () => toast.success("Trajet enregistre"),

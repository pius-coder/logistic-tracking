...
export const adminControlStepTimer = defineOperationFn("admin.controlStepTimer")
  ...
  .handler(async ({ ctx, input }) => {
    ...
    switch (input.action) {
      ...
      case "markReached": {
        // Mark this step as reached
        await ctx.db.trajectoryStep.update({
          where: { id: step.id },
          data: { reachedAt: now },
        });

        // Auto-start next step's timer if exists
        ...
        await ctx.db.jcNotification.create({
          data: {
            ...
            title: `Etape atteinte : ${step.locationName}`,
            message: `Votre expedition a atteint ${step.locationName}.`,
        ...
        await ctx.notify.via("whatsapp.send").send({
          phoneNumber: step.request.recipientPhone,
          message: `📍 JC Import Express — Etape atteinte\n\nN°: ${step.request.requestNumber}\nLieu: ${step.locationName}\n\nSuivez votre expedition:\n${trackingUrl}`,
        });
